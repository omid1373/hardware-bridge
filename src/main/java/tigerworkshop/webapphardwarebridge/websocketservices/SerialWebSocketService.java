    ...
    public void start() {
        readThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                System.out.println("Serial Read Thread started for " + portName);
...
//                            logger.info("Trying to connect the serial @ " + serialPort.getSystemPortName());
                            serialPort.openPort();
                        }
                    } catch (Exception e) {
        ...
        writeThread = new Thread(new Runnable() {
            @Override
            public void run() {
//                logger.info("Serial Write Thread started for " + portName);
                while (!Thread.interrupted()) {
                    if (serialPort.isOpen()) {
                        try {
                            // SAJAD
                            /*writeBuffer = "123".getBytes();
                            ThreadUtil.silentSleep(1000);*/
